...
$(function () {
	...
	function core (y) {
		if (y === '') {
		...
		} else if (y === 'hi' || y === 'hello' || y === 'hey' || y === 'greetings' || y.startsWith('hi ') || y.startsWith('hello ') || y.startsWith('hey ') || y.startsWith('greetings ')) {
			...
			if (y.indexOf('ti') !== -1) {
				...
				time = time.join(':');
				theme = (Math.random() < 0.25) ? '' : ' Try using the ' + ((now.getHours() <= 18 || now.getHours() >= 6) ? '"Light" or "Grassy"' : '"Dark" or "Space"') + ' theme if you aren\'t already.';
			}
			if (y.indexOf('d') !== -1) {
